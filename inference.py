...
# Compatibility shim: omegaconf>=2.1 renamed `is_primitive_type` to
# `is_primitive_type_annotation`, but the pinned fairseq still does a
# getattr/setattr dance on the old name inside load_checkpoint_to_cpu. Re-expose
# the old alias so checkpoint loading works with omegaconf 2.3.
import omegaconf._utils as _omegaconf_utils
if not hasattr(_omegaconf_utils, "is_primitive_type"):
    _omegaconf_utils.is_primitive_type = _omegaconf_utils.is_primitive_type_annotation
