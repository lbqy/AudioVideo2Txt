...
相对第 4 章 AV2T baseline 约提升 **+0.09** 个 valid acc 点。该提升幅度很小，后续若用于报告，
建议再补 best checkpoint 的测试集 WER，因为 teacher-forcing valid acc 的小差距未必能稳定转化为
自回归解码收益。

**结果分析**：

1. **整体收益很小**：AV2T baseline 本身已经有 91% 以上 valid acc，音频模态提供了强监督信号，
   干净验证集并不包含显式缺音频 / 缺视频扰动。Modality Dropout 的主要目标是提升模态缺失或局部遮挡
   时的鲁棒性，因此在 clean valid acc 上只能看到很小的正收益，甚至部分配置会略降。
2. **`modality_dropout=0.10` 最好但优势不大**：默认 0.05 的扰动略弱，0.10 增加了训练时对单模态
   依赖的抑制，使模型更愿意同时利用音频和视频；但 0.10 仍只是 10% 样本被扰动，没有破坏主任务学习，
   因此能得到当前最高的 91.578%。由于 0.05/0.10/0.03 之间差距仅 0.02~0.17 个点，可能存在随机种子
   和训练噪声影响，不宜把排序解释得过重。
