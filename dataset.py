...
import cupy
# Compatibility shim: python_speech_features_cuda references the legacy
# `cupy.core.core.ndarray`, which was removed in cupy>=10. Re-expose it so the
# library's runtime type check keeps working with cupy 12.x.
if not hasattr(cupy, "core"):
    import types as _types
    cupy.core = _types.SimpleNamespace(core=_types.SimpleNamespace(ndarray=cupy.ndarray))
...
class AudioVideoDataset(FairseqDataset):
    ...
    def get_label(self, index):
        ...
        # 1. read the raw label string located at (offset_s, offset_e) in the
        #    label file. The offsets are byte offsets (see load_label_offset).
        offset_s, offset_e = self.label_offsets_list[index]
        with open(self.label_path) as f:
            f.seek(offset_s)
            label = f.read(offset_e - offset_s).rstrip("\n")
        # 2. split the string into subwords with the BPE (sentencepiece) tokenizer
        label = self.bpe_tokenizer.encode(label.lower())
        # 3. encode each subword into a long index through the dictionary.
        #    append_eos=True so every target ends with <eos> (the model thus
        #    learns to terminate generation); add_if_not_exist=False so unknown
        #    subwords map to <unk> instead of extending the vocabulary.
    ...
    def load_video(self, video_name):
        ...
        # 1. read every frame of the video with cv2.VideoCapture and convert it
        #    to a single channel gray-scale image (opencv loads frames as BGR).
        ...
        while True:
            ...
            frames.append(frame)
        cap.release()
        # 2. stack the frames into a numpy array of shape [T, H, W] and apply the
        #    pre-defined data transform (normalization + cropping).
        feats = np.stack(frames, axis=0)  # [T, H, W]
        feats = self.transform(feats)
        # 3. add an artificial channel dimension -> [T, H, W, 1]
    ...
    def load_audio(self, audio_name):
        ...
        # 1. extract log Mel-filterbank features with logfbank. The signal is
        #    moved to the GPU as a cupy float32 array so the heavy spectrogram
        #    computation runs on GPU and offloads the CPU. logfbank returns
        #    (filterbank_energies, total_energy); we only keep the [T0, F0]
        #    filterbank energies (F0=26 by default).
        wav = cupy.asarray(wav_data, dtype=cupy.float32)
        feats = logfbank(wav, samplerate=sample_rate)[0]
        feats = feats.get() if hasattr(feats, "get") else np.asarray(feats)
        feats = feats.astype(np.float32)  # [T0, F0]

        # 2. stack every `stack_order_audio` consecutive frames together so that
        #    the audio frame rate matches the video frame rate. Zero-pad the tail
        #    when the number of frames is not a multiple of stack_order_audio.
        stack_order = self.stack_order_audio
        feat_dim = feats.shape[1]
        if len(feats) % stack_order != 0:
            res = stack_order - len(feats) % stack_order
            feats = np.concatenate(
                [feats, np.zeros([res, feat_dim], dtype=feats.dtype)], axis=0
            )
        # [T0, F0] -> [T, stack_order, F0] -> [T, stack_order * F0]
        audio_feats = feats.reshape(
