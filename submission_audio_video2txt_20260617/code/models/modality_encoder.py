...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        # first conv applies the (possibly down-sampling) stride, second conv
        # keeps the spatial size; both use 3x3 kernels.
        ...
        # downsample is used on the residual branch when the shape changes
    ...
    def forward(self, x):
        ...
        out = self.conv2(out)
        out = self.bn2(out)
        # if the residual branch changes shape (different channels / stride),
        # project the input with the downsample module before the addition.
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        # build the residual branch projection for the first block: it is needed
        # whenever the spatial resolution is reduced (stride != 1) or the channel
        # count changes (self.inplanes != planes * expansion).
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = self.downsample_block(
                self.inplanes, planes * block.expansion, stride
            )
        # only the first block performs (optional) down-sampling
        layers.append(block(self.inplanes, planes, stride, downsample))
        # subsequent blocks keep planes*expansion channels and stride 1
...
class AudioEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        # a single linear layer maps the raw audio feature dimension to the
    ...
    def forward(self, x):
        ...
        # input  x: [B, C, T] (C = audio_feat_dim, T = number of frames)
        # the linear layer acts on the feature dimension, so move it to the last
        # axis, project, then move it back. output: [B, encoder_embed_dim, T]
        x = x.transpose(1, 2)   # [B, T, C]
        x = self.proj(x)        # [B, T, output_dim]
        x = x.transpose(1, 2)   # [B, output_dim, T]
