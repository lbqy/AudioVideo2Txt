...
class AudioVideoBaseModelConfig(FairseqDataclass):
    ...
    modality_fuse: str = field(
        default="concat",
        metadata={"help": "modality fusion type: concat, add"},
    )
    modality_dropout: float = field(
        default=0.0,
        metadata={"help": "per-sample probability of applying modality dropout during AV training"},
    )
    audio_dropout: float = field(
        default=0.5,
        metadata={"help": "conditional probability of dropping audio after modality dropout selects a sample"},
    )
    modality_dropout_mode: str = field(
        default="sample",
        metadata={"help": "modality dropout mode: sample drops a full modality, span drops a time span"},
    )
    modality_dropout_span_min: int = field(
        default=10,
        metadata={"help": "minimum span length in frames for span modality dropout"},
    )
    modality_dropout_span_max: int = field(
        default=30,
        metadata={"help": "maximum span length in frames for span modality dropout"},
    )
...
class AudioVideoModelConfig(AudioVideoBaseModelConfig):
    ...
    ctc_weight: float = field(
        default=0.0,
        metadata={"help": "if > 0, build a CTC auxiliary projection head"},
    )


class EncoderBackboneWrapper(FairseqEncoder):
    def __init__(self, pretrained_model, apply_mask=False):
        super().__init__(None)
...
class AudioVideoModel(FairseqEncoderDecoderModel):
    def __init__(self, cfg, task_cfg):
        ...
        arg_overrides = {
            ...
            "modality_fuse": cfg.modality_fuse,
            "modality_dropout": cfg.modality_dropout,
            "audio_dropout": cfg.audio_dropout,
            "modality_dropout_mode": cfg.modality_dropout_mode,
            "modality_dropout_span_min": cfg.modality_dropout_span_min,
            "modality_dropout_span_max": cfg.modality_dropout_span_max,
        }
        ...
        utils.overwrite_args_by_name(pretrained_args.model, arg_overrides)

        encoder_ = EncoderBackbone(pretrained_args.model, pretrained_args.task, state['task_state']['dictionaries'])
        encoder = EncoderBackboneWrapper(encoder_, apply_mask=cfg.apply_mask)
        ...
        self.ctc_proj = (
            Linear(cfg.decoder_embed_dim, len(self.tgt_dict))
    ...
    def set_num_updates(self, num_updates):
        ...
        if hasattr(self.encoder, "pretrained_model") and hasattr(self.encoder.pretrained_model, "set_num_updates"):
            self.encoder.pretrained_model.set_num_updates(num_updates)
