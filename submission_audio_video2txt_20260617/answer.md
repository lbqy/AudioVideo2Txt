...
## 第 6 章 进一步优化方法：CTC 辅助监督

### 6.1 方法动机

第 5 章中的三种方法主要从损失重加权、模态扰动和特征遮挡角度进行正则化。实验结果显示，这些方法在
AV2T 任务上只有较小收益，甚至在 clean test WER 上不稳定。结合任务本身特点，音视频到文本识别具有较强
的单调对齐关系：输入的视频帧 / 音频帧时间顺序与输出文本顺序基本一致。因此，第 6 章采用
**CTC auxiliary loss** 作为进一步优化方法，在原 seq2seq decoder 交叉熵损失之外，对 encoder 输出增加
一条 CTC 监督分支，让模型在相同 `max_update=30000` 的有限训练步数内更快学习输入到输出的对齐关系。

该方法不增加训练数据、不引入外部语言模型，也不改变测试协议。总损失为：

```text
total_loss = seq2seq_ce_loss + ctc_weight * ctc_loss
```

其中 `ctc_weight` 为新增超参数。默认 baseline 中该参数为 `0.0`，因此不会影响第 4 章原始实验。

### 6.2 实现方式

**模型侧实现**位于 `models/audio_video_model.py`。在 `AudioVideoModelConfig` 中加入 `ctc_weight`，
当 `model.ctc_weight > 0` 时，模型额外构造一层 `ctc_proj`，将 encoder 输出从隐藏维度投影到目标词表维度。
原模型的 decoder 输出保持不变；开启 CTC 后，`forward` 返回包含 `decoder_out` 和 `encoder_out` 的结构，
供 criterion 计算辅助损失。

**损失侧实现**位于 `losses/label_smoothed_cross_entropy.py`。在原有 label-smoothed cross entropy 基础上，
加入可选 CTC loss：

1. `sample["target"]` 中去除 `<pad>` 和 `<eos>`，作为 CTC target；
2. 使用 `<pad>` 的 index 作为 CTC blank，避免修改词表和数据文件；
3. `input_lengths` 根据 encoder padding mask 计算，保证 padding 部分不参与 CTC 对齐；
4. `zero_infinity=True`，避免极短输入导致无穷 loss；
5. CTC logits 在计算 `log_softmax` 和 `ctc_loss` 前转为 float，以保证混合精度训练稳定。

训练日志额外记录 `ce_loss`、`ctc_loss` 和 `total_loss`，原有 `accuracy`、`nll_loss`、`valid_accuracy`
等指标保持不变。配置文件 `configs/audiovideo2text.yaml` 与 `configs/video2text.yaml` 中默认
`model.ctc_weight=0.0`、`criterion.ctc_weight=0.0`，只有实验脚本显式覆盖时才启用 CTC 分支。

AV2T 训练入口为 `scripts/train_ctc_aux_av2t.sh`，V2T 训练入口为 `scripts/train_ctc_aux_v2t.sh`。
第 6 章默认配置采用 AV2T、`ctc_weight=0.3`、`max_update=30000`。

### 6.3 实验结果

由于时间限制，第 6 章实际完成了三组 AV2T 训练，其中 `ctc_weight=0.1` 和 `ctc_weight=0.3`
完成了 `checkpoint_best.pt` 的测试集 WER 解码，`ctc_weight=0.5` 完成训练与验证但未进行测试集解码。

| 方法 | 模态 | `ctc_weight` | train acc | valid loss | valid acc | test WER |
| ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| 第 4 章 Baseline | AV2T | 0.0 | - | - | 91.49% | 13.853% |
| 5.1 损失函数改进 | AV2T | - | - | - | 90.946% | 13.772% |
| 5.2 最优扫参 | AV2T | - | 94.005% | 37.362 | 91.578% | 13.883% |
| 5.3 Feature Mask | AV2T | - | 87.686% | 37.415 | 91.265% | 14.951% |
| 6.1 CTC 辅助监督 | AV2T | 0.1 | 94.251% | 39.513 | 91.521% | 13.935% |
| 6.1 CTC 辅助监督 | AV2T | 0.3 | 94.359% | 43.316 | **91.583%** | **13.631%** |
| 6.1 CTC 辅助监督 | AV2T | 0.5 | 94.271% | 46.865 | 91.499% | 未解码 |

其中，`ctc_weight=0.3` 对应的测试集 WER 为 **13.6306%**，错误词数为 `1837 / 13477`；
第 4 章 AV2T baseline 的测试集 WER 为 **13.8532%**，错误词数为 `1867 / 13477`。因此，
CTC 辅助监督在相同 30000 step 训练预算下，将 AV2T 测试集 WER 降低了 **0.2226 个百分点**，
相对错误率下降约 **1.61%**，同时也超过了第 5 章中 WER 最好的 5.1 AV2T 结果。

主要实验产物如下：

| 实验 | checkpoint | WER 输出 |
| ---- | ---- | ---- |
| `ctc_aux_av2t_w01` | `exp/ctc_aux_av2t_w01/checkpoints/checkpoint_best.pt` | `results/inference/ctc_aux_av2t_w01/wer.244018` |
| `ctc_aux_av2t_w03` | `exp/ctc_aux_av2t_w03/checkpoints/checkpoint_best.pt` | `results/inference/ctc_aux_av2t_w03/wer.244018` |
| `ctc_aux_av2t_w05` | `exp/ctc_aux_av2t_w05/checkpoints/checkpoint_best.pt` | 未解码 |

### 6.4 结果分析

从结果看，`ctc_weight=0.3` 是当前最有效的第 6 章配置。它的 valid accuracy 为 **91.583%**，
略高于第 4 章 AV2T baseline 的约 91.49%，也高于 5.2 最优扫参的 91.578%；更重要的是，它在
自回归测试集 WER 上达到 **13.631%**，是目前所有已完成实验中最低的 WER。

`ctc_weight=0.1` 的 valid accuracy 达到 91.521%，但测试 WER 为 13.935%，低于 `ctc_weight=0.3`
且略差于 baseline。这说明过小的 CTC 权重虽然提供了辅助监督，但对 encoder 对齐学习的约束不够强，
不足以稳定改善最终解码结果。

`ctc_weight=0.5` 的 valid accuracy 为 91.499%，接近 baseline，但 valid loss 升高到 46.865。
这说明过强的 CTC 分支会显著改变总损失尺度，并可能与 decoder 的 seq2seq 学习目标产生竞争，使模型更多
优化单调对齐分支，而不是最终自回归生成目标。因此，第 6 章采用 `ctc_weight=0.3` 作为默认方案。

总体而言，CTC 辅助监督比第 5 章三种正则化方法更贴合音视频识别任务本身的时序对齐结构。它没有依赖更多
训练 step，而是在相同训练预算内给 encoder 增加直接的文本序列监督，使模型更快形成输入帧到输出 token
的单调对齐。最终实验表明，6.1 CTC 辅助监督是本项目目前最优的改进方法。
